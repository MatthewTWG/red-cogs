...
class Streams(commands.Cog):
    ...
    role_defaults = {
        "mention": False,
        "mention_twitch": None,  # None means use general mention setting
        "mention_youtube": None,  # None means use general mention setting
        "mention_picarto": None,  # None means use general mention setting
    }
    ...
    async def role(self, ctx: commands.Context, *, role: discord.Role):
        """Toggle a role mention for all stream types."""
    ...
    @mention.command(name="twitch")
    @commands.guild_only()
    async def role_twitch(self, ctx: commands.Context, *, role: discord.Role):
        """Toggle a role mention for Twitch streams specifically."""
        current_setting = await self.config.role(role).mention_twitch()
        if current_setting is None:
            # Not set, check general setting for context
            general_setting = await self.config.role(role).mention()
            await self.config.role(role).mention_twitch.set(not general_setting)
            if not general_setting:
                msg = _("{role} will now be mentioned for Twitch stream alerts.").format(
                    role=inline(f"@\u200b{role.name}")
                )
            else:
                msg = _("{role} will no longer be mentioned for Twitch stream alerts.").format(
                    role=inline(f"@\u200b{role.name}")
                )
        elif current_setting:
            await self.config.role(role).mention_twitch.set(False)
            msg = _("{role} will no longer be mentioned for Twitch stream alerts.").format(
                role=inline(f"@\u200b{role.name}")
            )
        else:
            await self.config.role(role).mention_twitch.set(True)
            msg = _("{role} will now be mentioned for Twitch stream alerts.").format(
                role=inline(f"@\u200b{role.name}")
            )

        if not role.mentionable and (current_setting is False or (current_setting is None and not await self.config.role(role).mention())):
            msg += " " + _(
                "Since the role is not mentionable, it will be momentarily made mentionable "
                "when announcing a Twitch streamalert. Please make sure I have the correct "
                "permissions to manage this role, or else members of this role won't receive "
                "a notification."
            )
        await ctx.send(msg)

    @mention.command(name="youtube")
    @commands.guild_only()
    async def role_youtube(self, ctx: commands.Context, *, role: discord.Role):
        """Toggle a role mention for YouTube streams specifically."""
        current_setting = await self.config.role(role).mention_youtube()
        if current_setting is None:
            # Not set, check general setting for context
            general_setting = await self.config.role(role).mention()
            await self.config.role(role).mention_youtube.set(not general_setting)
            if not general_setting:
                msg = _("{role} will now be mentioned for YouTube stream alerts.").format(
                    role=inline(f"@\u200b{role.name}")
                )
            else:
                msg = _("{role} will no longer be mentioned for YouTube stream alerts.").format(
                    role=inline(f"@\u200b{role.name}")
                )
        elif current_setting:
            await self.config.role(role).mention_youtube.set(False)
            msg = _("{role} will no longer be mentioned for YouTube stream alerts.").format(
                role=inline(f"@\u200b{role.name}")
            )
        else:
            await self.config.role(role).mention_youtube.set(True)
            msg = _("{role} will now be mentioned for YouTube stream alerts.").format(
                role=inline(f"@\u200b{role.name}")
            )
        ...
        if not role.mentionable and (current_setting is False or (current_setting is None and not await self.config.role(role).mention())):
            msg += " " + _(
                "Since the role is not mentionable, it will be momentarily made mentionable "
                "when announcing a YouTube streamalert. Please make sure I have the correct "
                "permissions to manage this role, or else members of this role won't receive "
                "a notification."
            )
        await ctx.send(msg)

    @mention.command(name="list")
    @commands.guild_only()
    async def mention_list(self, ctx: commands.Context):
        """List all role mention settings for stream alerts."""
        guild = ctx.guild
        role_settings = []

        for role in guild.roles:
            general = await self.config.role(role).mention()
            twitch = await self.config.role(role).mention_twitch()
            youtube = await self.config.role(role).mention_youtube()
            picarto = await self.config.role(role).mention_picarto()

            # Only show roles that have at least one mention setting enabled
            if general or twitch or youtube or picarto:
                platforms = []
                if twitch is True or (twitch is None and general):
                    platforms.append("Twitch")
                if youtube is True or (youtube is None and general):
                    platforms.append("YouTube")
                if picarto is True or (picarto is None and general):
                    platforms.append("Picarto")

                if platforms:
                    role_settings.append(f"**{role.name}**: {', '.join(platforms)}")

        if not role_settings:
            await ctx.send(_("No roles are currently set to be mentioned for stream alerts."))
        else:
            msg = _("**Current role mention settings:**\n\n") + "\n".join(role_settings)
            for page in pagify(msg, delims=["\n"]):
                await ctx.send(page)

    @mention.command(name="picarto")
    @commands.guild_only()
    async def role_picarto(self, ctx: commands.Context, *, role: discord.Role):
        """Toggle a role mention for Picarto streams specifically."""
        current_setting = await self.config.role(role).mention_picarto()
        if current_setting is None:
            # Not set, check general setting for context
            general_setting = await self.config.role(role).mention()
            await self.config.role(role).mention_picarto.set(not general_setting)
            if not general_setting:
                msg = _("{role} will now be mentioned for Picarto stream alerts.").format(
                    role=inline(f"@\u200b{role.name}")
                )
            else:
                msg = _("{role} will no longer be mentioned for Picarto stream alerts.").format(
                    role=inline(f"@\u200b{role.name}")
                )
        elif current_setting:
            await self.config.role(role).mention_picarto.set(False)
            msg = _("{role} will no longer be mentioned for Picarto stream alerts.").format(
                role=inline(f"@\u200b{role.name}")
            )
        else:
            await self.config.role(role).mention_picarto.set(True)
            msg = _("{role} will now be mentioned for Picarto stream alerts.").format(
                role=inline(f"@\u200b{role.name}")
            )
        ...
        if not role.mentionable and (current_setting is False or (current_setting is None and not await self.config.role(role).mention())):
            msg += " " + _(
                "Since the role is not mentionable, it will be momentarily made mentionable "
                "when announcing a Picarto streamalert. Please make sure I have the correct "
                "permissions to manage this role, or else members of this role won't receive "
                "a notification."
            )
        await ctx.send(msg)
                ...
                try:
                    ...
                    for channel_id in stream.channels:
                        ...
                        mention_str, edited_roles = await self._get_mention_str(
                            channel.guild, channel, guild_data, stream
                        )
    ...
    async def _get_mention_str(
        ...
        # Determine platform-specific mention key
        platform_mention_key = None
        if stream.platform_name == "Twitch":
            platform_mention_key = "mention_twitch"
        elif stream.platform_name == "YouTube":
            platform_mention_key = "mention_youtube"
        elif stream.platform_name == "Picarto":
            platform_mention_key = "mention_picarto"

        for role in guild.roles:
            # Check platform-specific mention setting first, fall back to general mention
            should_mention = False
            if platform_mention_key:
                platform_mention = await self.config.role(role).get_attr(platform_mention_key)()
                if platform_mention is not None:
                    should_mention = platform_mention
                else:
                    # Fall back to general mention setting
                    should_mention = await self.config.role(role).mention()
            else:
                # Unknown platform, use general mention setting
                should_mention = await self.config.role(role).mention()
